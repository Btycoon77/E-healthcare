...
$email=$_POST['email'];
$select_email_query="select email from doctor where email='$email' ";
	$run_email_query=mysqli_query($con,$select_email_query);
	
	if(mysqli_num_rows($run_email_query)>0)
	{
	$check_row= mysqli_fetch_assoc($run_email_query);
	
	$edit_email_query="update doctor set email='$email' where email='$email' ";
	
	
	$edit_run_email_query=mysqli_query($con,$edit_email_query);
	if(isset($edit_run_email_query))
	{
	$update_msg="updated ";
		echo"	<script> alert ('Are You Sure ? You Want To Change Your email')</script> ";
    ...
    <div class="card">
        <div class="info">
            ...
            <form action="#" method="post">
                ...
                <div class="input-box">
                        <span class="details">Previous Email</span>
